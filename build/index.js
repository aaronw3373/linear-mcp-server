...
class LinearMCPClient {
    ...
    async resolveTeamId(teamIdOrKey) {
        ...
        if (isUUID) return teamIdOrKey;
        const org = await this.rateLimiter.enqueue(() => this.client.organization);
        const teams = await org.teams();
        const match = teams.nodes.find(t => t.key === teamIdOrKey || t.name === teamIdOrKey);
        if (!match) throw new Error(`Team "${teamIdOrKey}" not found. Use team key (e.g. "WEI") or UUID.`);
        return match.id;
    }
    async resolveStateId(statusName, teamId) {
        const states = await this.rateLimiter.enqueue(() => this.client.workflowStates({
            filter: { name: { eq: statusName }, team: { id: { eq: teamId } } }
        }));
        if (!states.nodes.length) {
            const allStates = await this.rateLimiter.enqueue(() => this.client.workflowStates({
                filter: { name: { eq: statusName } }
            }));
            if (!allStates.nodes.length) throw new Error(`Status "${statusName}" not found in any workflow`);
            return allStates.nodes[0].id;
        }
        return states.nodes[0].id;
    }
    async resolveLabels(labelNames, teamId) {
        if (!labelNames || labelNames.length === 0) return [];
        const existingLabels = await this.rateLimiter.enqueue(() =>
            this.client.issueLabels({ filter: { team: { id: { eq: teamId } } }, first: 100 })
        );
        const labelMap = new Map(existingLabels.nodes.map(l => [l.name.toLowerCase(), l.id]));
        const ids = [];
        for (const name of labelNames) {
            ...
            if (existing) {
                ...
                const payload = await this.rateLimiter.enqueue(() =>
                    this.client.createIssueLabel({ name, color: '#6366f1', teamId })
                );
    ...
    async resolveProject(projectName) {
        ...
        const projects = await this.rateLimiter.enqueue(() =>
            this.client.projects({ filter: { name: { containsIgnoreCase: projectName } }, first: 5 })
        );
        if (!projects.nodes.length) throw new Error(`Project "${projectName}" not found. Create it first with linear_create_project.`);
        return projects.nodes[0].id;
    }
    async createLabel(args) {
        const teamId = await this.resolveTeamId(args.teamId);
        const payload = await this.rateLimiter.enqueue(() =>
            this.client.createIssueLabel({ name: args.name, color: args.color || '#6366f1', teamId })
        );
        const label = await payload.issueLabel;
    ...
    async createProject(args) {
        const teamId = await this.resolveTeamId(args.teamId);
        const payload = await this.rateLimiter.enqueue(() =>
            this.client.createProject({ name: args.name, teamIds: [teamId], description: args.description })
        );
    ...
    async createIssue(args) {
        ...
        if (args.status) {
            stateId = await this.resolveStateId(args.status, teamId);
        }
        let labelIds = undefined;
        if (args.labels && args.labels.length > 0) {
            labelIds = await this.resolveLabels(args.labels, teamId);
        }
        let projectId = undefined;
        if (args.project) {
            projectId = await this.resolveProject(args.project);
        }
    ...
    async updateIssue(args) {
        let issue;
        const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(args.id);
        if (isUUID) {
            issue = await this.client.issue(args.id);
        } else {
            const identifierMatch = args.id.match(/^([A-Z]+)-(\d+)$/);
            const filter = identifierMatch
                ? { number: { eq: parseInt(identifierMatch[2], 10) }, team: { key: { eq: identifierMatch[1] } } }
                : { title: { contains: args.id } };
            const result = await this.rateLimiter.enqueue(() => this.client.issues({
                filter,
                first: 1
            }));
            issue = result.nodes[0];
        }
        if (!issue)
        ...
        if (args.status) {
            const states = await this.rateLimiter.enqueue(() => this.client.workflowStates({
                filter: { name: { eq: args.status } }
            }));
            if (!states.nodes.length)
                throw new Error(`Status "${args.status}" not found in any workflow`);
            stateId = states.nodes[0].id;
        }
        let labelIds = undefined;
        if (args.labels && args.labels.length > 0) {
            const issueTeam = await this.rateLimiter.enqueue(() => issue.team);
            labelIds = await this.resolveLabels(args.labels, issueTeam?.id);
        }
        let projectId = undefined;
        if (args.project) {
            projectId = await this.resolveProject(args.project);
    ...
    async getUserIssues(args) {
        try {
            ...
            let result = await this.rateLimiter.enqueue(() => user.assignedIssues({
                first: args.limit || 50,
                includeArchived: args.includeArchived
            }));
            // Fall back to all issues if nothing is assigned (common when issues are unassigned)
            if (!result?.nodes || result.nodes.length === 0) {
                result = await this.rateLimiter.enqueue(() => this.client.issues({
                    first: args.limit || 50,
                    includeArchived: args.includeArchived,
    ...
    buildSearchFilter(args) {
        const filter = {};
        if (args.query) {
            // If query looks like a Linear identifier (e.g. "WEI-36"), match by number
            const identifierMatch = args.query.match(/^([A-Z]+)-(\d+)$/);
            if (identifierMatch) {
                filter.number = { eq: parseInt(identifierMatch[2], 10) };
                filter.team = filter.team || { key: { eq: identifierMatch[1] } };
            } else {
                filter.or = [
        ...
        if (args.teamId) {
            const isUUID = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(args.teamId);
            filter.team = isUUID ? { id: { eq: args.teamId } } : { key: { eq: args.teamId } };
        }
        if (args.status) {
...
const createIssueTool = {
    ...
    description: "Creates a new Linear issue. Required: title, teamId. Optional: description, priority (0-4), status, labels (array of label names, auto-created if missing), project (project name to assign to — must exist, create with linear_create_project).",
    inputSchema: {
        type: "object",
        properties: {
            ...
            labels: { type: "array", items: { type: "string" }, description: "Label names to apply (auto-created if they don't exist)" },
            project: { type: "string", description: "Project name to assign this issue to" }
        },
...
const updateIssueTool = {
    ...
    description: "Updates an existing Linear issue. Accepts issue ID or identifier (e.g. WEI-37). Optional fields: title, description, priority, status, labels (array of label names, auto-created if missing), project (project name).",
    inputSchema: {
        type: "object",
        properties: {
            id: { type: "string", description: "Issue ID or identifier (e.g. WEI-37)" },
...
const createLabelTool = {
    name: "linear_create_label",
    description: "Creates a new label in Linear for a given team. Labels can then be applied to issues via the labels field in linear_create_issue or linear_update_issue.",
    inputSchema: {
        type: "object",
        properties: {
            name: { type: "string", description: "Label name" },
            teamId: { type: "string", description: "Team ID" },
            color: { type: "string", description: "Hex color code (e.g. #6366f1). Defaults to indigo." }
        },
...
const createProjectTool = {
    ...
    description: "Creates a new Linear project (epic). Projects group related issues and show progress tracking. After creating, assign issues to it using the project field in linear_create_issue or linear_update_issue.",
    inputSchema: {
        type: "object",
        properties: {
...
const listProjectsTool = {
    ...
    description: "Lists all Linear projects. Use this to discover existing project names before assigning issues to them.",
    inputSchema: {
        type: "object",
        properties: {}
    }
...
const CreateIssueArgsSchema = z.object({
    ...
    labels: z.array(z.string()).optional().describe("Label names to apply (created automatically if they don't exist)"),
    project: z.string().optional().describe("Project name to assign this issue to (must exist — create with linear_create_project)")
});
const UpdateIssueArgsSchema = z.object({
    id: z.string().describe("Issue ID or identifier (e.g. WEI-37)"),
...
const CreateLabelArgsSchema = z.object({
    ...
    teamId: z.string().describe("Team ID"),
    color: z.string().optional().describe("Hex color code (e.g. #6366f1). Defaults to indigo.")
});
...
async function main() {
    try {
        ...
        server.setRequestHandler(ListToolsRequestSchema, async () => ({
            tools: [createIssueTool, updateIssueTool, searchIssuesTool, getUserIssuesTool, addCommentTool, createLabelTool, createProjectTool, listProjectsTool]
        }));
            ...
            try {
                ...
                switch (name) {
                    ...
                    case "linear_create_label": {
                        ...
                        const label = await linearClient.createLabel(validatedArgs);
                        return {
                            content: [{
                                    type: "text",
